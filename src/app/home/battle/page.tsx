...
import { useRouter } from 'next/navigation'; // FIX: Імпортуємо useRouter для навігації
...
const styles: { [key: string]: CSSProperties } = {
  ...
  storyContainer: { flex: 1, overflowY: 'auto', padding: '70px 20px 20px 20px', color: '#e0e7ff' },
  storyContent: { backgroundColor: 'rgba(10, 5, 20, 0.7)', backdropFilter: 'blur(10px)', padding: '20px', borderRadius: '12px', border: '1px solid rgba(129, 140, 248, 0.2)', fontSize: '18px', lineHeight: 1.7 },
  ...
  // FIX: Стиль для кнопки повернення
  backButton: {
    padding: "12px 20px",
    fontSize: 16,
    borderRadius: '10px',
...
export default function TextAdventurePage() {
  const router = useRouter(); // FIX: Ініціалізуємо роутер
...
The player is a Level ${playerData.level} ${playerData.character_class}. Start the story at a crossroad in a dark forest.`;
  ...
  const processOutcome = useCallback(async (outcome: Outcome | Outcome[]) => {
    // ... (весь код processOutcome залишається без змін)
    ...
    for (const singleOutcome of outcomes) {
        ...
        if (singleOutcome.type === 'REWARD' || singleOutcome.type === 'XP') {
            const { error } = await supabase.rpc('increment_user_stat', { 
                ...
                p_stat_name: singleOutcome.type === 'XP' ? 'experience' : singleOutcome.item, 
                p_increment_value: singleOutcome.amount 
            });

            if (error) {
                toast.error(`Помилка оновлення ${singleOutcome.type}`);
                console.error("Supabase RPC Error:", error);
            } else {
                let icon = '🎁';
                if(singleOutcome.type === 'REWARD') {
                    if(singleOutcome.item === 'points') icon = '🪨';
                    if(singleOutcome.item === 'atl_balance') icon = '🪙';
                    if(singleOutcome.item === 'ton_balance') icon = '💎';
                } else if (singleOutcome.type === 'XP') {
                    icon = '🔷';
                }
                toast.success(`Отримано: ${singleOutcome.amount} ${icon}!`);
            }
        }
  ...
  const handleChoice = useCallback(async (choice: string) => {
    // ... (весь код handleChoice залишається без змін)
    if (!playerData || chatHistory.length === 0) return;
    if (isInCombat && energy < 1) {
        toast.error("Недостатньо енергії ⚡");
        return;
    }
    
    setIsLoading(true);
    if (isInCombat) await spendEnergy(1);
  ...
  return (
    <div style={styles.pageContainer}>
        ...
        <footer style={styles.actionsContainer}>
            {/* FIX: Додано кнопку повернення додому */}
